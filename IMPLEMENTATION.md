...
The server rejects malformed JSON-RPC request lines, malformed or partial persisted JSONL records on reload, empty or whitespace-only task/action/guidance/evidence values, malformed JSON Outcome/Evidence values, invalid known scope field types/values, duplicate source experience ids, and identifiers longer than 256 characters. Optional string fields are type-checked instead of silently defaulting when a caller supplies another JSON type. Each Adaptation must cite at least one existing Experience. New Adaptations always start with zero external evidence; only `observe_adaptation` can add an observed result, and retired Adaptations cannot receive new observations. Observations must identify a project, task, or ecosystem when the adaptation scope requires them; out-of-scope observations are not counted. Every list tool has a hard maximum of 20 items, even when a larger limit is requested. Public responses project text and raw values to at most 512 characters/bytes with truncation metadata, including externally supplied observation results, expose evidence counts and source ids rather than full history, and show only five bounded recent observations; the append-only JSONL history retains the complete observed values.
